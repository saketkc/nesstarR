...
Reads `.Nesstar` binary containers from statistical agencies like [MOSPI
India](https://mospi.gov.in/) into R data frames and exports them to
CSV.
...
NESSTAR files embed Huffman-compressed XML with variable labels and
category codes:
...
Older NSS-format files lack XML blocks;
[`nesstar_metadata()`](https://saketkc.github.io/nesstarR/reference/nesstar_metadata.md)
returns `NULL` with a warning for those.

## Exporting to CSV

[`nesstar_export()`](https://saketkc.github.io/nesstarR/reference/nesstar_export.md)
writes each dataset to its own CSV file. It reads 50,000 rows at a time
by default, so large files don’t blow up your memory:
...
- Mode 5 (numerics): packed formats from 4-bit nibbles to IEEE 754
  `float64`, returned as `numeric`; missing values become `NA`

The parser auto-detects byte order (little-endian by default, big-endian
fallback) and converts all strings to UTF-8.
